...
第一个时钟上升沿：front给使能和第一个地址。icache接收信号，传给ram判断是否命中。

第二个时钟上升沿：front不管命中结果直接给第二个地址(因为第一个已经成功被我接收了，存在我这)。icache在posedge clk保存第一个地址备用。icache根据ram结果，通过组合逻辑判断命中，。若命中则向front输出stall=0，返回对应数据，同时接收下一个信号，传给ram判断是否命中；否则未命中，向mem访问数据，陷入等待，此时front持续给icache第二个地址，直到stall信号变回低电平的时钟周期，给出第三个地址，同时接收第二个地址的数据。
