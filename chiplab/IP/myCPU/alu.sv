...
(
    ...
    // final 
    bus32_t final_alu_res;
    logic[10:0] imm11;
    assign imm11 = ex_i.inst[25:15];
    logic [15:0] data0, data1, data2, data3;
    assign data0 = reg_data1[31:16];
    assign data1 = reg_data1[15:0];
    assign data2 = reg_data2[31:16];
    assign data3 = reg_data2[15:0];

    logic [4:0] clz0, clz1, clz2, clz3;
    logic [4:0] max_clz;

    clz u_clz0 (
        .clz_input({data0, 16'hffff}),
        .clz_out(clz0)
    );

    clz u_clz1 (
        .clz_input({data1, 16'hffff}),
        .clz_out(clz1)
    );

    clz u_clz2 (
        .clz_input({data2, 16'hffff}),
        .clz_out(clz2)
    );

    clz u_clz3 (
        .clz_input({data3, 16'hffff}),
        .clz_out(clz3)
    );

    logic [4:0] larger1, larger2;
    assign larger1 = clz0 > clz1 ? clz0 : clz1;
    assign larger2 = clz2 > clz3 ? clz2 : clz3;
    assign max_clz = larger1 > larger2 ? larger1 : larger2;


    bus32_t final_imm;
    assign final_imm = {{21{imm11[10]}}, imm11};
    assign final_alu_res = final_imm << (6'd32 - {1'b0, max_clz}) | final_imm >> max_clz;
    ...
    always_comb begin: reg_write
        case (ex_i.alusel)
            `ALU_SEL_FINAL: begin
                ex_o.reg_write_data = final_alu_res;
            end
